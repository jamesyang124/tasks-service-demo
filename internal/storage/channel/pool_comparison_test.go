...
// Benchmark ChannelStore performance
func BenchmarkChannelStore_Create(b *testing.B) {
...
func BenchmarkChannelStore_Read(b *testing.B) {
	store := NewChannelStore(4)
	defer store.Shutdown()
...
func BenchmarkChannelStore_Update(b *testing.B) {
	store := NewChannelStore(4)
	defer store.Shutdown()
